...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _login() async {
    ...
    final error =
        Validaciones.requerido(usuario, 'Ingresá tu correo o usuario.') ??
      ...
      setState(() {
        switch (e.code) {
          ...
          case 'ERROR_API':
            _errorMessage =
                e.message ??
                'El servidor rechazó la solicitud. Intentá de nuevo.';
            break;
          default:
            _errorMessage =
                e.message ?? 'No se pudo iniciar sesión (${e.code}).';
        }
      });
    } catch (e) {
      setState(() {
    ...
    try {
      ...
      setState(() {
        switch (e.code) {
          ...
          case 'sign_in_failed':
            _errorMessage =
                'No se pudo iniciar sesión con Google. Verificá la configuración de la cuenta.';
            break;
          case 'NEGOCIO':
            _errorMessage =
                e.message ?? 'Google no devolvió los datos necesarios.';
            break;
          default:
            _errorMessage =
                e.message ?? 'No se pudo continuar con Google (${e.code}).';
        }
      });
    } catch (e) {
