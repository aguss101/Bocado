...
Feed denormalizado. Columnas: `id_receta, nombre_receta, calorias_totales, porciones, foto, precio, precio_porcion, visibilidad, activo, lista_etiquetas (jsonb), lista_interacciones (jsonb), id_usuario, apellido_nombre, usuario, foto_perfil, cant_favoritos, cant_comentarios, promedio_calificacion, proteinas_totales, carbohidratos_totales, grasas_totales`.
> ✅ **Corrección 2026-07-04 (verificado por GET directo)**: la vista SÍ expone `activo`, `visibilidad` y `precio` (la doc vieja las omitía). Por eso `RecetaFeed.activo`/`visibilidad` llegan reales desde `mis_recetas_completo`, y el split Publicadas/Borradores del perfil y de MyRecipes funciona filtrando por `activo`.
...
| `obtener_perfil_editable` | `(p_id integer)` → TABLE(usuario, correo, id_genero, visibilidad) | 🆕 **2026-07-04 (2do lote)**. `SECURITY DEFINER SET search_path=public`. Reemplaza el GET REST `usuarios?select=usuario,correo,...` de `handleGetPerfilEditable` — con Fase 1, `anon` perdió el SELECT sobre `correo`, así que ese GET devolvía `42501` y rompía la pantalla Editar Perfil. La RPC corre como `postgres` y lee el correo del propio usuario. Devuelve solo esas 4 columnas (NO expone `contrasena`). |
...
6. ✅ **RESUELTO Y VERIFICADO EN VIVO (2026-07-04)** — RLS activado en todas las tablas (lectura pública donde corresponde, escritura cerrada por defecto — sin política = deny total). `pass_reset` cerrada del todo (ni lectura, confirmado: devuelve `[]` con HTTP 200). Vistas (`vistas_recetas_macros`, `vista_mis_seguidos`, `vista_comentarios_recetas`) pasadas a `security_invoker=true` — sin esto las vistas corren con los permisos del dueño (típicamente un rol que bypassea RLS) y la protección de las tablas de abajo no se aplicaría nunca al consultarlas.
   ...
   - ⚠️ **`SELECT *` sobre `usuarios` por REST quedó roto por Fase 1** (2do lote 2026-07-04): al sacar el GRANT de tabla completa y otorgar solo columnas puntuales a `anon`, un `select=*` requiere SELECT sobre TODAS las columnas (incluye `contrasena`/`correo` bloqueadas) → devuelve `42501`. Dos handlers de `AccessChannel` lo hacían: `handleGetPerfilUsuario` (ver perfil de un tercero — rompía con "Error al cargar el perfil" y volvía al perfil propio) se cambió a `select=` con lista explícita de columnas permitidas; `handleGetPerfilEditable` (pantalla Editar Perfil, necesita `correo`) se pasó a la RPC `obtener_perfil_editable` (`SECURITY DEFINER`). **Lección**: tras un lockdown de columnas, buscar y eliminar todo `select=*` sobre esa tabla por REST.
